...
type Cmd struct {
	...
	// Username specifies the username to log on as when starting
	// the command
	Username string

	// Domain specifies the domain to log on as when starting
	// the command
	Domain string
	...
	SysProcAttr *syscallex.SysProcAttr
...
func (c *Cmd) Start() error {
	...
	var err error
	c.Process, err = osex.StartProcessWithLogon(c.Path, c.argv(), c.Username, c.Domain, c.Password, &osex.ProcAttr{
		Dir:   c.Dir,
		Files: c.childFiles,
		Env:   dedupEnv(c.envv()),
		Sys:   c.SysProcAttr,
